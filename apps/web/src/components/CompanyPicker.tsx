...
export default function CompanyPicker({ value, onChange, placeholder, token }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`${apiUrl}/api/jobs/suggestions?field=company&q=${encodeURIComponent(input)}`, {
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        <input
          ref={inputRef}
          value={input}
          placeholder={tags.length === 0 ? (placeholder ?? 'Genesys, Five9, NICE…') : 'Add company…'}
          autoComplete="off"
          onChange={(e) => {
